...
class RunLookalike():
    def __init__(self,
        ...
        self.dynamic_context_col = ['interest1', 'interest2', 'interest3', 'kw1', 'kw2', 'topic1',
                                    'topic2']  # ['interest1', 'interest2', 'interest3', 'kw1', 'kw2', 'topic1', 'topic2']
        ...
        self.all_col = [self.label_col,
                        'aid'] + self.static_context_col + self.dynamic_context_col + self.col_length_name + self.ad_col
    ...
    def get_train_data(self):
        ...
        # 切换为小数据集 进行对齐时使用
        # data_train_stage1 = self.read_data('./data/processed_data/train_stage1.pkl')[self.all_col]
        # print('train stage1 {} '.format(data_train_stage1.shape[0]))

        # data_train_stage2 = self.read_data('./data/processed_data/train_stage1.pkl')[self.all_col]
        # print('train stage2 {} '.format(data_train_stage2.shape[0]))
    ...
    def get_model(self):
        if self.base_model_name == 'WD':
            model = MetaModel(col_names=self.columns, max_ids=self.max_ids, embed_dim=self.emb_dim,
                              ...
                              base_model_name=self.base_model_name, num_expert=self.num_expert,
                              num_output=self.num_output)
        else:
            raise ValueError('Unknown base model: ' + self.base_model_name)

        return model if self.use_cuda else model
    ...
    def get_optimizer(self, model):
        optimizer = paddle.optimizer.Adam(parameters=model.parameters(), learning_rate=self.local_test_lr,
    ...
    def train_stage(self, data_train_stage1, data_train_stage2, model, epoch):
        ...
        n_batch = int(np.ceil(n_samples / self.batchsize))  # 总量除以batchsize
        ...
        for i_batch in tqdm.tqdm(range(n_batch)):
            if (self.sample_method == 'normal') or (self.sample_method == 'sqrt'):
                batch_aid_set = np.random.choice(aid_set, size=self.task_count, replace=True,
                                                 p=list_prob)  # random.sample(aid_set, 5)
            elif self.sample_method == 'unit':
                batch_aid_set = random.sample(aid_set, self.task_count, replace=True)
    ...
    def test_train(self, data_train, model, criterion, optimizer, epoch):
        ...
        for i_batch in range(n_batch):
            ...
            label = paddle.to_tensor(batch_y.astype('float32'))
    ...
    def test_stage(self, data_test_stage1, data_test_stage2, model, criterion, stage):
        ...
        for aid in aid_set:
            ...
            model.set_state_dict(init_model)
        ...
        model.set_state_dict(init_model)
        result = 'Stage {}; AUC {}; GAUC {}; Precision {}; Recall {};\n'.format(stage, auc, gauc / len(aid_set),
                                                                                avg_precision / len(aid_set),
                                                                                avg_recall / len(aid_set))
        print(result)
        return result
    ...
    def main(self):
        ...
        model.set_state_dict(
            paddle.load('parameter_{}_{}_{}.pkl'.format(self.task_count, self.emb_dim, self.local_train_lr)))
